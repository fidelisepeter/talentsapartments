...
class Rooms
{
    ...
    public static function roomsAvalaible()
    {
        ...
        $bedSpace = BedSpace::all()->unique('room_id');
        foreach ($bedSpace as $view) {
            //     foreach (BedSpace::where('room_id', $view->room->id)->get()->unique('room_number') as $list) {
            ...
            //             $bedspace_count = BedSpace::where('room_id', $view->room->id)->where('room_number', $list->room_number)->get()->count();
            //             $roomView->bedspace_count = $bedspace_count;
            //             $resident_count = BedSpace::where('room_id', $view->room->id)->where('room_number', $list->room_number)->whereNotNull('user_id')->where('allocated', false)->get()->count();
            //             $get_residents = BedSpace::where('room_id', $view->room->id)->where('room_number', $list->room_number)->whereNotNull('user_id')->where('allocated', false)->get();
            ...
            foreach (BedSpace::where('room_id', $view->room->id)->get()->unique('room_number') as $list) {
                foreach (BedSpace::where('room_id', $view->room->id)->where('room_number', $list->room_number)->get()->unique('room_number') as $roomView) {
                    ...
                    $bedspace_count = BedSpace::where('room_id', $view->room->id)->where('room_number', $list->room_number)->get()->count();
                    $roomView->bedspace_count = $bedspace_count;
                    $resident_count = BedSpace::where('room_id', $view->room->id)->where('room_number', $list->room_number)->whereNotNull('user_id')->get()->count();
                    $get_residents = BedSpace::where('room_id', $view->room->id)->where('room_number', $list->room_number)->whereNotNull('user_id')->get();
                    $roomView->resident = $get_residents;
                    $roomView->with_resident = $get_residents;
                    $roomView->resident_count = $resident_count;
    ...
    public static function allRooms($type = null)
    {
        $get = [];
        if ($type != null) {
            $bedSpace = BedSpace::all()->where('room_id', $type)->unique('room_id');
        } else {
            $bedSpace = BedSpace::all()->unique('room_id');
        }

        foreach ($bedSpace as $space) {
            $spacelist = BedSpace::where('room_id', $space->room->id)->get()->unique('room_number');
            foreach ($spacelist as $list) {

                foreach (BedSpace::where('room_id', $space->room->id)->where('room_number', $list->room_number)->get()->unique('room_number') as $show) {
    ...
    //     $bedSpace = BedSpace::all()->unique('room_id');
    //     foreach($bedSpace as $space){
    //         $spacelist = BedSpace::whereNull('user_id')->where('room_id', $space->room->id)->get()->unique('room_number');
    //         foreach($spacelist as $list){
    ...
    public static function expiredInDays($init = 31, $room_id = null)
    {
        $rentByDays = [];

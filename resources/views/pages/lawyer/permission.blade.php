...
@endif
@section('page-title', $lawyer->first_name . ' ' . $lawyer->last_name . ' Permissions')
@section('content')
        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body p-3">
                    <div class="row">
                        {{-- @foreach ($lawyer->roles as $role_list)
                        ...
                        <div class="col-lg-3 col-6 text-center">
                            <div class="border-dashed border-1 border-secondary border-radius-md py-3">
                                ...
                                <h4 class="font-weight-bolder"><span id="state2"
                                        countto="2400">{{ $lawyer->roles()->first()->name ?? '' }}</span></h4>
                                <h6 class=" text-xs mb-0">{{ $lawyer->roles()->first()->permissions->count() ?? '' }} permission
                                </h6>
    ...
    {{-- <div class="row mt-3">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body p-3">
                    <form action="/lawyer/{{ $lawyer->id }}/role/update" method="POST">
                        @csrf
                        <select class="multisteps-form__select form-control" name="role" onchange="this.form.submit()">
                            <option value="">-select-</option>
                            @foreach ($roles as $role)
                                <option value="{{ $role->name }}" @if ($lawyer->hasRole($role->name)) selected @endif>
                                    {{ $role->name }}</option>
                            @endforeach
                        </select>
                    </form>
    ...
    <form action="/lawyer/{{ $lawyer->id }}/permission/update" method="POST">
        @csrf
        <div class="row mt-3">
                                                ...
                                                <li class="list-group-item border-0 px-0"
                                                    ...
                                                    <div class="form-check form-switch ps-0">
                                                        <input class="form-check-input ms-auto" type="checkbox"
                                                            ...
                                                            @if ($lawyer->can($category_permission->name)) checked="" @endif
                                                        ...
                                                        <label
                                                            ...
                                                            for="flexSwitchCheckDefault">{{ Str::ucfirst(str_replace('-', ' ', $category_permission->name)) }}</label>
                                                    </div>
                                                </li>
                                            @endif
                                        @endforeach
            ...
            <div class="col-12">
                <button type="submit" class="btn bg-gradient-info mb-0">Update Lawyer Permission</button>
            </div>
